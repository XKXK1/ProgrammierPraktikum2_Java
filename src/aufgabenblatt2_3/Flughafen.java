...
public class Flughafen extends Thread {
  ...
  private int zeit;
  private String[] fluggesellschaft = {"Lufthansa", "Air Berlin", "Air France",
   ...
   * Dauerschleife, bis der Thread durch interrupt() oder einen Programmabbruch
   * beendet wird. Die Schleife erstellt solange Flugzeugobjekte die zuvor
   * uebergebene Zahl der maximalen Flugzeuge erreicht ist. Sobald ein Flugzeug
   * gelandet ist wird ein neues Flugzeugobjekt erstellt. Im sekundentakt wir
   * der Status des Flughafens ausgegeben.
   */
  @Override
  public void run() {
   ...
   * Flugzeug-Objektes. Es wird ein zufaelliger Fluggesellschaftsname aus einem
   * String[] gewaehlt,eine zufaellige Flugzeugnummer und eine zufaellige
   * flugdauer hinzugefuegt. Als Argument wird der Flughafen und die Startzeit
   * des Flugzeugs erwartet. Am Ende der Methode wird ein Flugzeug-Objekt
   * zurueckgegeben.
   * 
   * @param flughafen
   * @param startzeit
  ...
  private Flugzeug erzeugeFlugzeug(Flughafen flughafen, int startzeit) {
    ...
    String flugzeugName = fluggesellschaft[(int) (Math.random()
        * fluggesellschaft.length)] + " " + (int) (Math.random() * 9000);
